...
- [ ] **Regenerate the LOM reference XML against Live 12 using
      `API_MakeDoc`.** Current reference at `tools/ref/Live11.0-LOM.xml`
      is Live 11.0 (Julien Bayle's structure-void output). Live 12
      release notes confirm no envelope/automation additions in
      12.0–12.3.7 but other Live 12 LOM changes are possible. Drop
      `API_MakeDoc` (the same Remote Script that produced the structure-
      void XMLs) into Live's MIDI Remote Scripts folder, select it as a
      control surface, it writes `Live<version>.xml` to disk. Check the
      output in at `tools/ref/Live12.<version>-LOM.xml`. Prereq for the
      LOM audit item below.
      Scope: docs/ref (one-shot artifact, no code)

- [ ] **Audit Live LOM and backlog high-value tool gaps.** Once the
      Live 12 reference XML lands (item above), read class-by-class,
      identify Live features the MCP doesn't yet expose that would help
      music-production agents (candidates: locator/cue points, groove
      pool, scenes, return-track sends, time signature changes, track
      freezing, MIDI CC assignments, warp markers, follow-actions, tempo
      automation). Add discrete items per feature. The output of this
      audit *is* the next wave of backlog items — schedule it before
      picking up further feature work so we're not building blind.
