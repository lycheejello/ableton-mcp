...
- [ ] **Audit Live LOM and backlog high-value tool gaps.** Reference copy
      ...
      other Live 12 LOM changes are possible — ideally regenerate against
      our Live 12 build for a definitive reference. Goal: read class-by-
      class, identify Live features the MCP doesn't yet expose that would
      help music-production agents (candidates: locator/cue points,
      groove pool, scenes, return-track sends, time signature changes,
      track freezing, MIDI CC assignments, warp markers, follow-actions,
      tempo automation). Add discrete items per feature.
